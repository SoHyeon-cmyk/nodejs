  ...
  return (
    ...
    <div className="App">
      ...
      </div>
      {
        ...
        <h4 onClick={()=>{setModal(!modal)}} style={{color:'blue', fontSize:'20px'}}>{item} 
          <span style={{color:'red'}} onClick={(e)=>{ e.stopPropagation(); setPostLikes(i,likes[i]+1)}}> ❤</span> {likes[i]}
          </h4>
        <h4>글제목</h4>
        <p>2024년 12월 11일</p>
        <button onClick={()=>{
...

      {
        ...
        <button onClick={()=> { let copy =[...post]; copy.unshift(input);setPost(copy)}}>글쓰기</button>
        </>
      }
